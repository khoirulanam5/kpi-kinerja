...
class Nilai_realisasi_pimpinan extends CI_Controller {
	...
	public function index()
	{
		...
		if($jabatan == 'Manajer Pemasaran' || $jabatan == 'Supervisor ADK' || $jabatan == 'Supervisor Pely Intern' || $jabatan == 'Supervisor Pely Kas' || $jabatan == 'Supervisor Adm Unit') {
			...
			$data['daftar_karyawan'] = $this->m_users->ambil_karyawan_byjb2('Karyawan', 'Pemagang');
		}elseif($jabatan == 'AMPB') {
	...
	public function daftar_nilai($id_karyawan, $nama)
	{
		...
		$data['bulan_realisasi'] = $this->m_kpi->ambil_bulan_kpi_luar($id_karyawan);

		$this->compose_view('pimpinan/bulan_realisasi', $data);
	}
	...
	public function edit_nilai($bulan, $id_karyawan)
	{
		$data['title'] = "Detail Nilai Realisasi";
		$data['active'] = "Nilai Realisasi";
		$data['bulan'] = $bulan;
		$data['id_karyawan'] = $id_karyawan;
		$data['nilai_realisasi'] = $this->m_kpi->ambil_detail_kpi_luar($id_karyawan, $bulan);
	...
	public function edit_nilai_form($bulan, $id_karyawan) {
		...
		redirect('nilai_realisasi_pimpinan/edit_nilai/'.$bulan.'/'.$id_karyawan);
	}
	...
	public function detail_realisasi($bulan, $id_karyawan)
	{
		$data['title'] = "Detail Nilai Realisasi";
		$data['active'] = "Nilai Realisasi";
		$data['bulan'] = $bulan;
